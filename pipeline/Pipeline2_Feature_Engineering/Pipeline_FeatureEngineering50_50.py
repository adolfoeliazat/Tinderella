...
class Feature_Engineer(object):
    ...
    def __init__(self, stand_img_directory, cached_feature_vector_file,
        ...
        """
        inputs:
             main directory containing all lable directories with
             shape: tuple: current shape of images
             target_shape: tuple: target matrix size to run machine
             learning on.
    ...
    def pre_trans(self, img_arr, img_file_path):
        ...
        pre_trans = np.zeros(
            4 * (img_arr.shape[0] * img_arr.shape[1] * img_arr.shape[2]))
        ...
        pre_trans[prior_length:prior_length + stand_feat_det_img_arr.shape[0]]
    ...
    def create_feature_vector(
        ...
        """
          ...
          #column wise concatenate flattened filtered image to
          label, pre_features and post_features arrays.
    ...
    def feature_preprocessing(self):
        ...
        label_vec = []
        f = open(
            '%s/size_new_twenty_50_50_10e_labels.csv' %
            FeatVecs_path,
            'w')
        t = open('%s/size_new_twenty_50_50_10e_items.csv' % FeatVecs_path, 'w')
        non = open(
            '%s/size_new_twenty_50_50_10e_fails.csv' %
            FeatVecs_path,
            'w')
        ...
        for i, subdir in enumerate(clean_stand_img_directory_lst):
            ...
            for j, img_file in enumerate(clean_img_lst):
                ...
                try:
                    pre_trans_feat, feat_det_size = self.pre_trans(
                        img_arr, img_file_path)
                ...
                unflattened_trans_img_arr = self.filter_transform(
                ...
                try:
                    ...
                    post_trans_feat, post_feat_det_size =
                    self.post_trans(unflattened_trans_img_arr)
                ...
                feat_vector = self.create_feature_vector(
                    pre_trans_feat,
                    post_trans_feat,
        ...
        np.save('%s/rescaled_new_feat_matrix_50_50_10e.npy'
                % FeatVecs_path, rescaled_feat_matrix)
...
if __name__ == '__main__':
    ...
    FeatVecs_path = os.path.join(
        '/Volumes/hermanng_backup/Virginia_Capstone', 'FeatVecs')

    if not os.path.exists(FeatVecs_path):
        print FeatVecs_path
        os.mkdir(FeatVecs_path)

    cach_FeatVecs_path = os.path.join(FeatVecs_path,
                                      'size_new_twenty_50_50_10e_feat/')
