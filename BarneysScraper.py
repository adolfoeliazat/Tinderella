...
class BarneysScraper(object):
	...
	def get_page_links(self):
		...
		total_num_products = re.search(r"(?P<num_products>\d+) Products",total_products).group("num_products")
	...
	def _get_img(self, img_tags, category, orientation='front'):
		...
		for i, tag in enumerate(img_tags):
			...
			product_id = re.search(r"\/(?P<product_id>\d+)\_(\w+)", img_link).group("product_id")
			img_lst.append(img)
	...
	def _get_page_content(self, link):
		...
		for item in description_tags:
			product_id.append(re.search(r"\-(?P<product_id>\d+)\.",item['href']).group("product_id"))
