...
class KMeansClustering(object):
	...
	def train_test_split(self):
		...
		self.train_x, self.test_x, self.train_y, self.test_y = 
		      train_test_split(self.feat_matrix, self.target_arr, test_size=.2, random_state=1)
	...
	def Random_ForestClass(self):
		...
		print 'avg f1 cross_val: ', sum(self.cross_val(rf,self.test_x[:,trans_start:trans_end]
			  ,self.test_y ))/5.
...
if __name__ == '__main__':
	...
	with open('FeatVecs/size_new_twenty_50_50_10e_test15k_labels.csv', 'r') as f:
		y = f.read().split(',')
